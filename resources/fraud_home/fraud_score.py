...
def f1(df: pd.DataFrame, class_column: str, label_column: str):
    ...
    f1_sum = sum(f1_df['weight*x'].values) / sum(f1_df['subtotal'].values)

    print('f1 value  %.4f' % f1_sum)
    return f1_sum, df
...
def f2(df: pd.DataFrame, class_column: str, label_column: str):
    ...
    f2_sum = sum(f2_df['weight*x'].values) / sum(f2_df['subtotal'].values)

    print('f2 value  %.4f' % f2_sum)
    return f2_sum


def fraud_score(df: pd.DataFrame, class_column: str, label_column: str, beta=2, treshold=0.75):
