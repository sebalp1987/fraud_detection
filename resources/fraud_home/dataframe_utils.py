...
from fraud_home.resources.fraud_home import write_csv


class DfUtils:

    def load_df_file(self: pd.DataFrame):
        """
    ...
    def statistic_df(self: pd.DataFrame, output=False):
    ...
    def df_fillna(self: list, df, value):
        """
    ...
    def string_categoric(self: list, var_grouped: str, df, type_agg='count'):
        """
    ...
    def values_variables(self: pd.DataFrame, path):
        """
        ...
        for i in self.columns:
            if (self[i].dtype == np.float64 or self[i].dtype == np.int64):
                values = self[i].unique()

                list_values.append([i, values])

        write_csv.WriteCsv.write_csv(list_values, path)

    def processing_file(self, delimiter=',', nan_values='?'):
        """
    ...
    def processing_file_without_header(self, delimiter=',', first_row=True):
        """
        It loads and gives a csv format to the BOTTLE raw file
        """
